 ...
 * @constructor
 */
const ListBooks = ({books, addToShelf}) =>
{
	ListBooks.propTypes = {
			...
			<div className="list-books-content">
				<div>{
					DISPLAY_BOOKSHELVES.map((bookshelf, index) =>
						<Bookshelf key={index} bookshelfTitle={bookshelf.title} books={books.filter((book) => book.shelf === bookshelf.shelf)} addToShelf={addToShelf}/>
					)
				}
				</div>
			</div>
